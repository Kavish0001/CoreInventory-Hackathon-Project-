...
const Deliveries = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await inventoryService.listDeliveries({ q });
    ...
    <div>
      <div className="flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">Track outgoing stock deliveries by status.</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        {loading ? (
          <div className="p-6 text-gray-500">Loading deliveries...</div>
        ) : view === 'list' ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  ...
                  <th className="px-6 py-4 text-sm font-semibold text-gray-600">To (Customer)</th>
              ...
              <tbody>
                {filteredRows.length === 0 ? (
                  <tr>
                    <td colSpan="5" className="px-6 py-12 text-center text-gray-500 italic">No delivery orders found.</td>
                  </tr>
                ) : (
                  filteredRows.map((r) => (
        ...
        {!loading && view === 'list' ? (
          <div className="md:hidden p-4 space-y-3">
            {filteredRows.length === 0 ? (
              <div className="text-sm text-gray-500 italic">No delivery orders found.</div>
            ) : (
              filteredRows.map((r) => (
                <div key={r.id} className="border border-gray-200 rounded-xl p-4 bg-white">
                  <div className="flex justify-between items-start gap-3">
                    <div>
                      ...
                      <div className="text-sm text-gray-600 mt-1">{r.customer}</div>
                    </div>
                    <span className={statusBadge(isLate(r) ? 'late' : r.status)}>{isLate(r) ? 'late' : r.status}</span>
                  </div>
